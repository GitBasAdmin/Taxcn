...
export class Fud0100Tab4Component implements OnInit {
  ...
  gotoFudPage(type:any){
    let winURL = window.location.href.split("/#/")[0]+"/#/";
    if(type==1){
      var name = 'fud0110';
      myExtObject.OpenWindowMaxName(winURL+'fud0110?in_out=1&run_id='+this.dataHead.run_id+'&appeal_running='+this.sendEditData[0].appeal_running,name);
    }else{
      var name = 'fud0111';
      myExtObject.OpenWindowMaxName(winURL+'fud0111?in_out=2&run_id='+this.dataHead.run_id+'&appeal_running='+this.sendEditData[0].appeal_running,name);
    }
  }
  ...
  loadData(){
    ...
    if(typeof this.dataHead !='undefined'){
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiUD/API/APPEAL/fud0100/getAppealDoc', student ).subscribe(
        (response) =>{
                    ...
                    const subscription2 = confirmBox2.openConfirmBox$().subscribe(resp => {
                        if (resp.success==true){
                          ...
                          if(this.modalType==1){
                            ...
                            console.log(data)
                            this.http.post('/'+this.userData.appName+'ApiUD/API/APPEAL/fud0100/delAppealDoc', data).subscribe(
                              (response) =>{
                                let getDataOptions : any = JSON.parse(JSON.stringify(response));
                                console.log(getDataOptions)
