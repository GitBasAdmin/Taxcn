...
@Component({
  selector: 'app-fud0113',
  templateUrl: './fud0113.component.html',
  styleUrls: ['./fud0113.component.css']
})


export class Fud0113Component implements AfterViewInit, OnInit, OnDestroy {
  //form: FormGroup;
  title = 'datatables';
  sessData:any;
  userData:any;
  ...
  getData(){
    ...
    console.log(student)
    this.http.post('/'+this.userData.appName+'ApiUD/API/APPEAL/fud0113/getData', student ).subscribe(
      (response) =>{
    ...
    saveData(){
      ...
      if(bar){
        if(!dataTmp.length){
          ...
          console.log(data)
          this.http.post('/'+this.userData.appName+'ApiUD/API/APPEAL/fud0113/saveData', data ).subscribe(
            (response) =>{
              let getDataOptions : any = JSON.parse(JSON.stringify(response));
              console.log(getDataOptions)
              if(getDataOptions.result==1){
