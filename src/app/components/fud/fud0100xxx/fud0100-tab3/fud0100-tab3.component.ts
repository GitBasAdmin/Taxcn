...
import { Fud0130Component } from '@app/components/fud/fud0130/fud0130.component';
import { NgbModal, NgbModalRef   } from '@ng-bootstrap/ng-bootstrap';
declare var myExtObject: any;
@Component({
  selector: 'app-fud0100-tab3',
  templateUrl: './fud0100-tab3.component.html',
  styleUrls: ['./fud0100-tab3.component.css'],
  encapsulation: ViewEncapsulation.None,
})
export class Fud0100Tab3Component implements OnInit {
  ...
  gotoFnoPage(){
    ...
    var name = 'fno0900';
    myExtObject.OpenWindowMaxName(winURL+'fno0900?run_id='+this.dataHead.run_id+'&appeal_running='+this.sendEditData[0].appeal_running,name);
  }

  gotoFudPage(){
    let winURL = window.location.href.split("/#/")[0]+"/#/";
    var name = 'fud0114';
    myExtObject.OpenWindowMaxName(winURL+'fud0114?run_id='+this.dataHead.run_id+'&appeal_running='+this.sendEditData[0].appeal_running,name);
  }
  ...
  loadData(val:any){
    ...
    if(typeof this.dataHead !='undefined'){
      var student = JSON.stringify({
        ...
        "n_type" : val?val:this.result.n_type3,
                    ...
                    const subscription2 = confirmBox2.openConfirmBox$().subscribe(resp => {
                        if (resp.success==true){
                          ...
                          if(this.modalType==1){
                            ...
                            console.log(data)
                            this.http.post('/'+this.userData.appName+'ApiUD/API/APPEAL/fud0100/delAppealNotice', data).subscribe(
                              (response) =>{
                                let getDataOptions : any = JSON.parse(JSON.stringify(response));
                                console.log(getDataOptions)
                                ...
                                if(getDataOptions.result==1){
                                    ...
                                    const subscription3 = confirmBox3.openConfirmBox$().subscribe(resp => {
                                      if (resp.success==true){
                                        //this.setDefPage();
                                        this.loadData(this.result.n_type3)
                                      }
  ...
  fud0130Page(i:any){
    ...
    let winURL = window.location.href.split("/#/")[0]+"/#/";
    var name = 'fud0130';
    myExtObject.OpenWindowMaxName(winURL+'fud0130?run_id='+this.sendEditData[0].run_id+'&appeal_running='+this.sendEditData[0].appeal_running+'&notice_running='+this.AppealReqData[i].notice_running+'&n_type='+this.result.n_type3,name);
    */
    const modalRef: NgbModalRef = this.ngbModal.open(Fud0130Component,{ windowClass: 'my-class'})
    modalRef.componentInstance.run_id = this.sendEditData[0].run_id
    modalRef.componentInstance.appeal_running = this.sendEditData[0].appeal_running
    modalRef.componentInstance.notice_running = this.AppealReqData[i].notice_running
    modalRef.componentInstance.n_type = this.result.n_type3
