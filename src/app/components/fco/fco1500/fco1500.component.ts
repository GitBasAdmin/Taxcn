...
@Component({
  selector: 'app-fco1500,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fco1500.component.html',
  styleUrls: ['./fco1500.component.css']
})


export class Fco1500Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    searchData(type:any){
      ...
      if((!this.result.sapp_date || !this.result.eapp_date) && (!this.result.sdate || !this.result.edate)){
        ...
        let headers = new HttpHeaders();
          headers = headers.set('Content-Type','application/json');
        this.http.post('/'+this.userData.appName+'ApiCO/API/CORRESP/fco1400/searchData', data , {headers:headers}).subscribe(
          (response) =>{
...
tabChangeInput(id:any,name:any,event:any){
  ...
  exportAsXLSX(): void {
    if(this.dataSearch){
      ...
      if(bar){
        ...
        this.excelService.exportAsExcelFile(objExcel,'fco1500' ,this.programName);
      }
  ...
  printReport(size:any, run_seq:any){
    var rptName = 'rco0500';
