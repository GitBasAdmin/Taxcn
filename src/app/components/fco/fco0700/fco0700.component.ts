...
@Component({
  selector: 'app-fco0700,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fco0700.component.html',
  styleUrls: ['./fco0700.component.css']
})


export class Fco0700Component implements AfterViewInit, OnInit, OnDestroy {
  //form: FormGroup;
  title = 'datatables';
  ...
  // @ViewChild('fco0700',{static: true}) case_type : ElementRef;
 ...
 ngAfterViewInit(): void {
  ...
  ngOnDestroy(): void {
    ...
    searchData(){
      ...
      if(this.result.must_reply != true){
        this.result.rep_con = null;
      }
      if(!this.result.case_type &&
        ...
        !this.result.must_reply && !this.result.doc_dest && !this.result.rep_con
         ){
      ...
      //   });
        ...
        */
        ...
        this.http.post('/'+this.userData.appName+'ApiCO/API/CORRESP/fco0700', student , {headers:headers}).subscribe(
          (response) =>{
              let productsJson : any = JSON.parse(JSON.stringify(response));
              if(productsJson.result==1){
                ...
                var bar = new Promise((resolve, reject) => {
                  productsJson.data.forEach((ele, index, array) => {
                        ...
                        // }
                        if(ele.black_no != null){
                          if(ele.black_no.length > 15 ){
                             productsJson.data[index]['black_no_short'] = ele.black_no.substring(0,15)+'...';
                          }else{
                             productsJson.data[index]['black_no_short'] = ele.black_no;
                          }
                        }
                        if(ele.red_no != null){
                          if(ele.red_no.length > 15 ){
                            productsJson.data[index]['red_no_short'] = ele.red_no.substring(0,15)+'...';
                          }else{
                            productsJson.data[index]['red_no_short'] = ele.red_no;
                          }
                        }
                        // if(ele.deposit != null){
    ...
    exportAsXLSX(): void {
      if(this.dataSearch){
        ...
        var bar = new Promise((resolve, reject) => {
          ...
          for(var i = 0; i < excel.length; i++){
            ...
            for(var x=0;x<21;x++){
              ...
              if(x==4)
                data.push(excel[i]['doc_id_title']);
              if(x==5)
                data.push(excel[i]['doc_org']);
              if(x==6)
                data.push(excel[i]['doc_orig']);
              if(x==7)
                data.push(excel[i]['doc_dest']);
              if(x==8)
              ...
              if(x==11)
                data.push(excel[i]['thing1']);
              if(x==12)
                data.push(excel[i]['must_reply']);
              if(x==13)
                data.push(excel[i]['return_back_date']);
              if(x==14)
                data.push(excel[i]['reply_doc_no']);
              if(x==15)
        ...
        if(bar){
          ...
          this.excelService.exportAsExcelFile(objExcel,'fco0700' ,this.programName);
        }
