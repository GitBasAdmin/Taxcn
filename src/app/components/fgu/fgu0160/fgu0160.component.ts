...
@Component({
  selector: 'app-fgu0160,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fgu0160.component.html',
  styleUrls: ['./fgu0160.component.css']
})


export class Fgu0160Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  public loadFgu0160Component : boolean = false;
  ...
  ngOnInit(): void {
    ...
    //   this.http.disableLoading().post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0160/getGuarantorData', student).subscribe(
    //     (response) =>{
...
this.getSalagType = [{id: '1',text: 'สลาก'},{id: '2',text: 'พันธบัตร'}];
this.result.ssalag_type = 'สลาก';
this.getAssetId();
 ...
 ngAfterViewInit(): void {
  ...
  receiveFuncListData(event:any){
    ...
    if(this.listTable=='pguarantor_detail'){
      ...
      this.result.sbank_id = event.fieldIdValue;
      this.result.sbank_name = event.fieldNameValue;
    }
...
// }
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0160/search', student , {headers:headers}).subscribe(
        posts => {
...
closeModal(){
  ...
  getData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.SpinnerService.show();
      this.http.get('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0160?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          console.log(productsJson)
          this.posts = productsJson.data;
          console.log(this.posts);
