...
@Component({
  selector: 'app-fgu0180,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fgu0180.component.html',
  styleUrls: ['./fgu0180.component.css']
})


export class Fgu0180Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  public loadFgu0180Component : boolean = false;
  ...
  ngOnInit(): void {
    ...
    //   this.http.disableLoading().post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0180/getAssetData', student).subscribe(
    //     (response) =>{
...
this.getIType = [{id: 1,text: 'หลังกระทำผิด'},{id: 2,text: 'ก่อนกระทำผิด'}];
this.result.itype = 1;
this.getAssetId();
 ...
 ngAfterViewInit(): void {
  ...
  loadMyModalComponent(){
    ...
    if(this.modalType==1){
      ...
      this.listFieldCond="";
        ...
        $("#exampleModaltab1").find(".modal-content").css("width","650px");
        var student = JSON.stringify({
            "table_name" : "pins_comp",
            "field_id" : "ins_id",
            "field_name" : "ins_name",
            "userToken" : this.userData.userToken
          });
        this.listTable='pins_comp';
        this.listFieldId='ins_id';
        this.listFieldName= 'ins_name';
        this.listFieldName2='';
        this.listFieldCond="";
    ...
    if(this.modalType==1 || this.modalType==2 || this.modalType==3 || this.modalType==4 || this.modalType==5){
      console.log(student)
  ...
  receiveFuncListData(event:any){
    ...
    if(this.listTable=='pguarantor_detail'){
      ...
      this.result.bbook = event.fieldIdValue;
      this.result.bank_name = event.fieldNameValue;
    }else if(this.listTable =='pins_comp'){
      this.result.ins_id = event.fieldIdValue;
      this.result.icompany = event.fieldNameValue;
    }
...
// }
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0180/search', student , {headers:headers}).subscribe(
        posts => {
...
closeModal(){
  ...
  getData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.SpinnerService.show();
      this.http.get('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0180?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          console.log(productsJson)
          this.posts = productsJson.data;
          console.log(this.posts);
