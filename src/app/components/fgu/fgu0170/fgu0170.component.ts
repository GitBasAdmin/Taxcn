...
@Component({
  selector: 'app-fgu0170,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fgu0170.component.html',
  styleUrls: ['./fgu0170.component.css']
})


export class Fgu0170Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  public loadFgu0170Component : boolean = false;
  ...
  ngOnInit(): void {
    ...
    //   this.http.disableLoading().post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0170/getGuarantorData', student).subscribe(
    //     (response) =>{
...
// }
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0170/search', student , {headers:headers}).subscribe(
        posts => {
...
closeModal(){
  ...
  getData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.SpinnerService.show();
      this.http.get('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0170?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          console.log(productsJson)
          this.posts = productsJson.data;
          console.log(this.posts);
