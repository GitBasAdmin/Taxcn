...
@Component({
  selector: 'app-fgu0190,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fgu0190.component.html',
  styleUrls: ['./fgu0190.component.css']
})


export class Fgu0190Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  public loadFgu0190Component : boolean = false;
  ...
  ngOnInit(): void {
    ...
    // });
    //   console.log('getAssetData==>',student);
    //   this.SpinnerService.show();
    //   this.http.disableLoading().post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0190/getAssetData', student).subscribe(
    //     (response) =>{
    ...
    //         //   this.posts.forEach((x : any ) => x.edit0190 = false);
    //         // this.rerender();
    //       }else{
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0190 = this.masterSelected;
      }
  ...
  tabChangeGuar(objName:any,objData:any,event:any,type:any){
    ...
    if(typeof objData!='undefined'){
      ...
      console.log(val);
      if(val.length!=0){
        // this.result[objName] = val[0].guarman_item;
        // this[objName] = val[0].guarman_item;
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0190 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0190 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
...
// }
  ...
  editData(index:any){
    ...
    this.SpinnerService.show();
      ...
      this.hid_asset_item  = this.posts[index]['edit_asset_item'];
      this.result.edit_guarman_tiem = this.posts[index]['edit_guarman_item'];
      this.result.edit_asset_tiem = this.posts[index]['edit_asset_item'];
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0190/search', student , {headers:headers}).subscribe(
        posts => {
...
closeModal(){
  ...
  getData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.SpinnerService.show();
      this.http.get('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0190?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          console.log(productsJson)
          this.posts = productsJson.data;
          console.log(this.posts);
