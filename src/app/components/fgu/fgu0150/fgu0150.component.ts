...
@Component({
  selector: 'app-fgu0150,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fgu0150.component.html',
  styleUrls: ['./fgu0150.component.css']
})


export class Fgu0150Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  public loadFgu0150Component : boolean = false;
  ...
  ngOnInit(): void {
    ...
    //   this.http.disableLoading().post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0150/getGuarantorData', student).subscribe(
    //     (response) =>{
...
// }
  ...
  editData(index:any){
    ...
    this.SpinnerService.show();
      ...
      // this.result.ptambon_id = this.posts[index]['tambon_id'];
      // this.result.pamphur_id = this.posts[index]['amphur_id'];
      this.result.pprov_id = this.posts[index]['prov_id'];
      if(this.result.pprov_id)
      this.changeProv(this.result.pprov_id,'');
       else{
  ...
  AssignData(obj:any){
    ...
    var objData = obj;
      ...
      this.result.pprov_id = objData[0].pprov_id;
      if(this.result.pprov_id)
      this.changeProv(this.result.pprov_id,'');
       else{
        this.sAmphur.clearModel();
      ...
      this.edit_amphur_id = objData[0].pamphur_id;
      this.edit_tambon_id = objData[0].ptambon_id;
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0150/search', student , {headers:headers}).subscribe(
        posts => {
    ...
    (response) =>{
      ...
      setTimeout(() => {
        if(this.edit_amphur_id)
          this.result.pamphur_id = this.edit_amphur_id;
        if(this.edit_amphur_id){
          this.changeAmphur(this.result.pamphur_id,type);
        }
      }, 100);
...
changeAmphur(Amphur:any,type:any){
  ...
  var student = JSON.stringify({
    ...
    "condition" : " AND amphur_id='"+Amphur+"' AND prov_id='"+this.result.pprov_id+"'",
    "userToken" : this.userData.userToken
  });
  //console.log(student)
  this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student , {headers:headers}).subscribe(
    (response) =>{
      ...
      setTimeout(() => {
        if(this.edit_tambon_id)
          this.result.ptambon_id = this.edit_tambon_id;
      }, 100);
    },
    (error) => {}
  )
...
changeTambon(Tambon:any,type:any){
  ...
  var student = JSON.stringify({
    ...
    "condition" : " AND tambon_id='"+Tambon+"' AND amphur_id='"+this.result.pamphur_id+"' AND prov_id='"+this.result.pprov_id+"'",
    "userToken" : this.userData.userToken
  });
  //console.log(student);
  this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student , {headers:headers}).subscribe(
    (response) =>{
      ...
      setTimeout(() => {
        this.result.post_code = this.getPostNo.find((o:any) => o.fieldIdValue === this.result.ptambon_id).fieldNameValue2;
      }, 100);
    },
    (error) => {}
  )
...
closeModal(){
  ...
  getData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.SpinnerService.show();
      this.http.get('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0150?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          console.log(productsJson)
          this.posts = productsJson.data;
          console.log(this.posts);
