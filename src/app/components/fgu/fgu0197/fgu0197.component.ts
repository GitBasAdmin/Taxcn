...
@Component({
  selector: 'app-fgu0197,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fgu0197.component.html',
  styleUrls: ['./fgu0197.component.css']
})


export class Fgu0197Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  public loadFgu0197Component : boolean = false;
  ...
  ngOnInit(): void {
    ...
    //   this.http.disableLoading().post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0197/getGuarantorData', student).subscribe(
    //     (response) =>{
...
// }
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0197/search', student , {headers:headers}).subscribe(
        posts => {
...
closeModal(){
  ...
  getData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.SpinnerService.show();
      this.http.get('/'+this.userData.appName+'ApiGU/API/GUARANTEE/fgu0197?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          console.log(productsJson)
          this.posts = productsJson.data;
          console.log(this.posts);
