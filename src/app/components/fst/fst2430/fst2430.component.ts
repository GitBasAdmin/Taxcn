...
export class Fst2430Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  insertData(){
    console.log('Insert==>',this.posts);
      ...
      let promise = new Promise((resolve, reject) => {
        this.http.post('/'+this.userData.appName+'ApiST/API/STAT/fst2430/saveData', data).subscribe(
        (response) =>{
          let getDataOptions : any = JSON.parse(JSON.stringify(response));
  ...
  updateData(){
    ...
    var del = 0;
    var bar = new Promise((resolve, reject) => {
    console.log('Update==>',this.dataSearch);
    // return false;
  ...
  assingData(getDataOptions :any){
    ...
    var bar = new Promise((resolve, reject) => {
          getDataOptions.data.forEach((ele, index, array) => {
            ...
            if(ele.judge_id != null){
              getDataOptions.data[index]['edit2430'] = false;
              console.log(getDataOptions.data[index]['edit2430']);
            }
        });
    });
    ...
    //     this.posts.data.forEach((x : any ) => x.edit2430 = false);
    // console.log(this.posts);
...
getPosts(post_id:any,modalType:any){
  ...
  confirmData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    console.log(this.posts)
    if(!this.posts.case_type  || !this.posts.stat_mon  || !this.posts.stat_year || !this.posts.create_user){
      ...
      const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
        if (resp.success==true){
          ...
          this.http.post('/' + this.userData.appName + 'ApiST/API/STAT/fst2430/confirmData', student).subscribe(
            (response) => {
              let alertMessage: any = JSON.parse(JSON.stringify(response));
              if (alertMessage.result == 1) {
                confirmBox.setMessage(alertMessage.error);
    ...
    if(!this.posts.case_type  || !this.posts.stat_mon  || !this.posts.stat_year || !this.posts.create_user){
      ...
      const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
        ...
        if (resp.success==true){
          ...
          this.http.post('/' + this.userData.appName + 'ApiST/API/STAT/fst2430/cancelData', student).subscribe(
            (response) => {
              let alertMessage: any = JSON.parse(JSON.stringify(response));
              if (alertMessage.result == 1) {
                confirmBox.setMessage(alertMessage.error);
