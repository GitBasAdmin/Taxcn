...
@Component({
  selector: 'app-fst0110,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fst0110.component.html',
  styleUrls: ['./fst0110.component.css']
})


export class Fst0110Component implements AfterViewInit, OnInit, OnDestroy {
  form: FormGroup;
  title = 'datatables';
  ...
  @ViewChild('fst0110',{static: true}) fst0110 : ElementRef;
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.get('/'+this.userData.appName+'ApiST/API/STAT/fst0110?userToken='+this.userData.userToken+':angular').subscribe(posts => {
 ...
 ngAfterViewInit(): void {
  ...
  setFocus(name:any) {
    const ele = this.fst0110.nativeElement[name];
    if (ele) {
      ele.focus();
    }
  }
  ...
  submitForm() {
    ...
    if(!this.result.case_type){
      ...
      confirmBox.setMessage('กรุณาระบุอักษรคำนำหน้าคดีแดง');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
      ...
      if(this.edit_case_type && this.edit_case_type_stat && this.edit_case_title && this.edit_title_type){
        // console.log("update=>");
        this.http.post('/'+this.userData.appName+'ApiST/API/STAT/fst0110/updateJson', student ).subscribe(
          (response) =>{
        ...
        //Insert
        // console.log("save=>");
        this.http.post('/'+this.userData.appName+'ApiST/API/STAT/fst0110/saveJson', student ).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
            // console.log("alertMessage.result=>", alertMessage.result);
              if(alertMessage.result==0){
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      var student = JSON.stringify({
        "case_type" : this.fst0110.nativeElement["case_type"].value,
        "case_type_stat" : this.fst0110.nativeElement["case_type_stat"].value,
        "case_title" : this.fst0110.nativeElement["case_title"].value,
        "userToken" : this.userData.userToken
      });
      // console.log(student)
      this.http.post('/'+this.userData.appName+'ApiST/API/STAT/fst0110/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          ...
          if(productsJson.result==1){
            this.checklist = this.posts;
              if(productsJson.length)
                this.posts.forEach((x : any ) => x.edit_edit0110 = false);
              this.rerender();
          }else{
            confirmBox.setMessage(productsJson.error);
            confirmBox.setButtonLabels('ตกลง');
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                    ...
                    // console.log(data)
                    this.http.post('/'+this.userData.appName+'ApiST/API/STAT/fst0110/deleteSelect', data ).subscribe((response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
                      }else{
        ...
        (response) =>{
          ...
          setTimeout(() => {
            if(this.edit_case_title &&! this.result.case_title && type==""){
              ...
              this.result.case_title = this.userData.defaultRedTitle;
            }
          }, 100);
        },
        (error) => {}
