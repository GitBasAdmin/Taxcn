...
@Component({
  selector: 'app-fst0210,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fst0210.component.html',
  styleUrls: ['./fst0210.component.css']
})


export class Fst0210Component implements AfterViewInit, OnInit, OnDestroy {
  //form: FormGroup;
  myExtObject:any;
  title = 'datatables';
  toPage:any='fmg0200';
  ...
  ngOnDestroy(): void {
    ...
    var bar = new Promise((resolve, reject) => {
      ...
      if (!del) {
        ...
        var bar = new Promise((resolve, reject) => {
          this.dataSearch.forEach((ele, index, array) => {
                if(ele.edit0210 == true){
                  ...
                  delete tmp.judging_date;//ไม่ส่งค่า judging_date
                  dataTmp.push(tmp);
        ...
        this.SpinnerService.show();
          ...
          this.http
            .post('/'+this.userData.appName+'ApiST/API/STAT/fst0210/saveData', data, {
    ...
    searchData(){
      ...
      if(!this.result.case_type &&
        ...
        const confirmBox = new ConfirmBoxInitializer();
        confirmBox.setTitle('ข้อความแจ้งเตือน');
        confirmBox.setMessage('ป้อนวันที่ออกแดงให้ครบ');
        confirmBox.setButtonLabels('ตกลง');
        confirmBox.setConfig({
            layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
        });
        ...
        this.http.post('/'+this.userData.appName+'ApiST/API/STAT/fst0210', student ).subscribe(
          (response) =>{
              let productsJson : any = JSON.parse(JSON.stringify(response));
