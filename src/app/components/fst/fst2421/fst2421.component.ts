...
@Component({
  ...
  templateUrl: './fst2421.component.html',
  styleUrls: ['./fst2421.component.css']
})


export class Fst2421Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  caseTypeStat(){
    ...
    this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student).subscribe(
      (response) =>{
        ...
        this.cTypeHead.unshift({fieldIdValue:0,fieldNameValue: 'ภาค'});
  ...
  buttProcess(){
    ...
    if(!this.result.stat_mon){
      ...
      this.http.disableLoading().post('/'+this.userData.appName+'ApiST/API/STAT/fst2421/getStatData', student ).subscribe(
        (response) =>{
  ...
  submitForm(){
    ...
    var data =  this.posts;
    this.http.disableLoading().post('/'+this.userData.appName+'ApiST/API/STAT/fst2421/saveData', data ).subscribe(
      (response) =>{
        let alertMessage : any = JSON.parse(JSON.stringify(response));
          if(alertMessage.result==0){
            this.SpinnerService.hide();
  ...
  searchData(){
    ...
    if(!this.result.stat_mon){
      ...
      this.http.post('/'+this.userData.appName+'ApiST/API/STAT/fst2421', student ).subscribe(
        (response) =>{
          let alertMessage : any = JSON.parse(JSON.stringify(response));
            if(alertMessage.result==0){
              this.SpinnerService.hide();
    ...
    if(!this.posts.stat_mon || !this.posts.stat_year || !this.posts.create_user){
      ...
      const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
        if (resp.success==true){
          ...
          this.http.post('/' + this.userData.appName + 'ApiST/API/STAT/fst2421/confirmData', student).subscribe(
            (response) => {
              let alertMessage: any = JSON.parse(JSON.stringify(response));
    ...
    if(!this.posts.stat_mon || !this.posts.stat_year || !this.posts.create_user){
      ...
      const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
        ...
        if (resp.success==true){
          ...
          this.http.post('/' + this.userData.appName + 'ApiST/API/STAT/fst2421/cancelData', student).subscribe(
            (response) => {
              let alertMessage: any = JSON.parse(JSON.stringify(response));
              if (alertMessage.result == 1) {
                confirmBox.setMessage(alertMessage.error);
