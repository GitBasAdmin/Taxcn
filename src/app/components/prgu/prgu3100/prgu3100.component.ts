...
@Component({
  selector: 'app-prgu3100,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './prgu3100.component.html',
  styleUrls: ['./prgu3100.component.css']
})


export class Prgu3100Component implements AfterViewInit, OnInit, OnDestroy {
  myExtObject :any;
  //form: FormGroup;
  title = 'datatables';
  ...
  @ViewChild('prgu3100',{static: true}) prgu3100 : ElementRef;
  ...
  ngOnInit(): void {
    ...
    this.getPayType = [{id:'0',text:'ไม่ระบุ'},{id:'1',text:'ชำระทั้งหมด'},{id:'2',text:'ผ่อนชำระ'},{id:'3',text:'บังคับคดี'}];
    this.getPorder = [{id:'1',text:'ชื่อนายประกัน'},{id:'2',text:'เลขประกัน'}];
    this.setDefPage();
  }
  ...
  setDefPage(){
    this.result = [];
    this.result.pflag=1;
    this.result.ppay_type = '0';
    this.result.porder_by = '1';
  }
    ...
    printReport(){
      ...
      // For set parameter to report
       var paramData = JSON.stringify({
        ...
        "pflag" : this.result.pflag ? this.result.pflag : '1',
        "ppay_type" : this.result.ppay_type ? this.result.ppay_type.toString() : '',
        "porder_by" : this.result.porder_by ? this.result.porder_by.toString() : '',
        "ptitle_id" : this.result.ptitle_id ? this.result.ptitle_id.toString() : '',
       });
