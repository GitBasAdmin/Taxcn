...
@Component({
  selector: 'app-prgu2600,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './prgu2600.component.html',
  styleUrls: ['./prgu2600.component.css']
})


export class Prgu2600Component implements AfterViewInit, OnInit, OnDestroy {
  myExtObject :any;
  //form: FormGroup;
  title = 'datatables';
  ...
  @ViewChild('prgu2600',{static: true}) prgu2600 : ElementRef;
  ...
  ngOnInit(): void {
    ...
    this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student , {headers:headers}).subscribe(
      (response) =>{
        let getDataOptions : any = JSON.parse(JSON.stringify(response));
        this.getPeriodName = getDataOptions;
        },
        (error) => {}
      )
    //----------------------end getPeriodName -------------------
    ...
    this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student , {headers:headers}).subscribe(
      (response) =>{
        let getDataOptions : any = JSON.parse(JSON.stringify(response));
        this.getPermit = getDataOptions;
        this.getPermit.unshift({fieldIdValue:0,fieldNameValue: 'ทั้งหมด'});
        },
        (error) => {}
      )
    //----------------------end getPermit -------------------
  ...
  printReport(){
      ...
      // For set parameter to report
       var paramData = JSON.stringify({
         ...
         "pguar_period" : this.result.guar_period ? this.result.guar_period.toString() : '',
         "pguar_permit" : this.result.pguar_permit ? this.result.pguar_permit.toString() : '0',
       });
