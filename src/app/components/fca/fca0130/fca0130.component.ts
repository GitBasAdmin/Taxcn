...
@Component({
  selector: 'app-fca0130,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fca0130.component.html',
  styleUrls: ['./fca0130.component.css']
})


export class Fca0130Component implements AfterViewInit, OnInit, OnDestroy,AfterContentInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      this.litType = parseInt(params['lit_type']);
  ...
  this.runSeq(this.dataLitigant.lit_type);
}
 ...
 ngAfterViewInit(): void {
  ...
  /*
 sendDataToParent(runId:any){
   //console.log(runId)
   this.counter = this.counter+1;
   var f = {"run_id":runId,"counter":this.counter};
  parent.postMessage(f,location.origin);
 }
 */
  ...
  onClickSaveData(data:any,event:any): void {
    ...
    var lit_type = (typeof data.lit_type == "undefined" || data.lit_type == null) ? 0 : data.lit_type;
          ...
          if(alertMessage.result==0){
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
              if (resp.success==true){
                //this.rerender();
                if(event==1)
                  this.reloadNewPage();//location.reload();
                else
                  this.runId = this.dataHead.run_id;
              }
              subscription.unsubscribe();
  ...
  fnDataHead(event:any){
    console.log(event)
    this.dataHead = event;
    this.c_title = this.dataHead.title;
    this.c_id = this.dataHead.id;
    this.c_yy = this.dataHead.yy;
  ...
  searchIdCard(){
    if((this.dataLitigant.card_type==1 && this.dataLitigant.pers_type==2 )&&(this.idCard0 && this.idCard1 && this.idCard2 && this.idCard3 && this.idCard4)){
    ...
    }else if(this.dataLitigant.pers_type==1){//นิติบุคคล
      console.log(this.idCard)
      var dataDel = [];
        dataDel['id_card'] = this.idCard;
        dataDel['userToken'] = this.userData.userToken;
        var data = $.extend({}, dataDel);
        console.log(data)
        this.http.post('/'+this.userData.appName+'ApiCA/API/CASE/findLitigant', data ).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
            ...
            if(alertMessage.result==1){
              let getDataOptions : any = JSON.parse(JSON.stringify(alertMessage.data[0]));
              this.dataLitigant = getDataOptions;
              console.log(this.dataLitigant.id_card);
              // if(this.dataLitigant.id_card){
              //   this.idCard = this.dataLitigant.id_card;
              // }
              this.title = '';
  ...
  receiveFuncLawyerData(event:any){
    ...
    this.dataLitigant.addr_no = event.address;
    this.dataLitigant.birth_date = event.birth_date;
    this.dataLitigant.nation_id = event.nation_id;
  ...
  clearData(){
    ...
    if(run_id)
      location.replace(winURL+'fca0130?run_id='+run_id)
    else
      location.replace(winURL+'fca0130')
    window.location.reload();
  }
}
