...
@Component({
  selector: 'app-fca0231,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fca0231.component.html',
  styleUrls: ['./fca0231.component.css']
})


export class Fca0231Component implements AfterViewInit, OnInit, OnDestroy {
  //form: FormGroup;
  title = 'datatables';
  ...
  // @ViewChild('fca0231',{static: true}) case_type : ElementRef;
  ...
  ngOnDestroy(): void {
    ...
    searchData(){
      ...
      if(!this.result.case_cate_id &&
        !this.result.attach_type_id &&
        !this.result.file_desc &&
        !this.result.sdate &&
        !this.result.edate){
      ...
      }else if((!this.result.sdate || !this.result.edate) ){
        const confirmBox = new ConfirmBoxInitializer();
        confirmBox.setTitle('ข้อความแจ้งเตือน');
        confirmBox.setMessage('เลือกวันที่แนบไฟล์');
        confirmBox.setButtonLabels('ตกลง');
        confirmBox.setConfig({
            layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
        });
        ...
        */
        ...
        this.http.post('/'+this.userData.appName+'ApiCA/API/CASE/fca0231', student , {headers:headers}).subscribe(
          (response) =>{
    ...
    clickOpenFile(index:any,type:any){
      ...
      winURL = winURL+'/'+this.userData.appName+"ApiCA/API/CASE/fca0230/openAttach";
      console.log("http://"+winURL+'?run_id='+this.dataSearch[index].run_id+'&file_name='+this.dataSearch[index].file_name)
      myExtObject.OpenWindowMax("http://"+winURL+'?run_id='+this.dataSearch[index].run_id+'&file_name='+this.dataSearch[index].file_name);
  }
