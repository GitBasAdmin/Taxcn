...
export class Fno2310Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  printReport(type:any){
    ...
    var paramData = JSON.stringify({
      "pevent_date" : myExtObject.conDate(this.result2.sevent_date),
  ...
  setDefPage(){
    ...
    this.result2.sevent_date = myExtObject.curDate();
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                    ...
                    console.log(data)
                    this.http.post('/'+this.userData.appName+'ApiNO/API/NOTICE/fno2310/cancelHistoricalData', data , {headers:headers}).subscribe(
                      (response) =>{
  ...
  searchNotice(type:any){
    const confirmBox = new ConfirmBoxInitializer();
      ...
      if(type==1){
        ...
        let headers = new HttpHeaders();
        headers = headers.set('Content-Type','application/json');
        this.http.post('/'+this.userData.appName+'ApiNO/API/NOTICE/fno2310/insertHistoricalFromBarcode', student , {headers:headers}).subscribe(
          (response) =>{
            let productsJson : any = JSON.parse(JSON.stringify(response));
            console.log(productsJson)
            if(productsJson.result==1){
  ...
  searchNoticeHistoricalData(){
    this.SpinnerService.show();
    var student = JSON.stringify({
      "sevent_date" : this.result2.sevent_date,
      "userToken" : this.userData.userToken
    });
    console.log(student)
    let headers = new HttpHeaders();
    headers = headers.set('Content-Type','application/json');
    this.http.post('/'+this.userData.appName+'ApiNO/API/NOTICE/fno2310/getHistoricalData', student , {headers:headers}).subscribe(
      (response) =>{
        let productsJson : any = JSON.parse(JSON.stringify(response));
        console.log(productsJson)
        this.SpinnerService.hide();
  ...
  searchNoticeData(){
    ...
    let headers = new HttpHeaders();
    headers = headers.set('Content-Type','application/json');
    this.http.post('/'+this.userData.appName+'ApiNO/API/NOTICE/fno2310/getNoticeData', student , {headers:headers}).subscribe(
      (response) =>{
        let productsJson : any = JSON.parse(JSON.stringify(response));
        console.log(productsJson)
        if(productsJson.result==1){
    ...
    console.log(this.result.send_flag)
      ...
      if(bar){
        if(!dataTmp.length){
          ...
          console.log(data)
          this.http.post('/'+this.userData.appName+'ApiNO/API/NOTICE/fno2310/insertHistoricalData', data , {headers:headers}).subscribe(
            (response) =>{
              let alertMessage : any = JSON.parse(JSON.stringify(response));
              console.log(alertMessage)
              if(alertMessage.result==0){
