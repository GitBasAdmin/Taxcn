...
@Component({
  selector: 'app-fno1000v10',
  templateUrl: './fno1000v10.component.html',
  styleUrls: ['./fno1000v10.component.css']
})
export class Fno1000v10Component implements AfterViewInit, OnInit, OnDestroy {
  sessData:any;
  userData:any;
  litType:any;
  getAppeal:any;
  ...
  clickOpenMyModal(type:any){
    ...
    if(type==1){
      ...
      var student = JSON.stringify({
        "table_name" : "pprison_type",
         "field_id" : "prison_type_id",
         "field_name" : "prison_type",
        "userToken" : this.userData.userToken
      });    
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student ).subscribe(
        ...
        console.log(productsJson)
          ...
          const modalRef = this.ngbModal.open(DatalistReturnComponent)
            modalRef.componentInstance.items = productsJson
            modalRef.componentInstance.value1 = "pprison_type"
            modalRef.componentInstance.value2 = "prison_type_id"
            modalRef.componentInstance.value3 = "prison_type"
            modalRef.componentInstance.types = "1"
            modalRef.result.then((item: any) => {
              if(item){
                this.result.prison_type_id=item.fieldIdValue;
                this.result.prison_type=item.fieldNameValue;
              }
            })
        },
        (error) => {}
  ...
  tabChangeInput(name:any,event:any){
    ...
    }else if(name=='prison_type_id'){
      var student = JSON.stringify({
        "table_name" : "pprison_type",
        "field_id" : "prison_type_id",
        "field_name" : "prison_type",
        "condition" : " AND prison_type_id='"+event.target.value+"'",
        "userToken" : this.userData.userToken});
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student).subscribe(
        (response) =>{
        ...
        console.log(productsJson)
        if(productsJson.length){
          this.result.prison_type = productsJson[0].fieldNameValue;
        }else{
          this.result.prison_type_id = this.result.prison_type = '';
        }
        },
        (error) => {}
      )
