...
@Component({
  selector: 'app-fju0500,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fju0500.component.html',
  styleUrls: ['./fju0500.component.css']
})


export class Fju0500Component implements AfterViewInit, OnInit, OnDestroy {
  //form: FormGroup;
  title = 'datatables';
  ...
  // @ViewChild('fju0500',{static: true}) case_type : ElementRef;
    ...
    searchData(){
      ...
      if(!this.result.case_type &&
        ...
        */
        ...
        this.http.post('/'+this.userData.appName+'ApiJU/API/JUDGEMENT/fju0500', student , {headers:headers}).subscribe(
          (response) =>{
              let productsJson : any = JSON.parse(JSON.stringify(response));
    ...
    exportAsXLSX(): void {
      if(this.dataSearch){
        ...
        if(bar){
          ...
          this.excelService.exportAsExcelFile(objExcel,'fju0500' ,this.programName);
        }
