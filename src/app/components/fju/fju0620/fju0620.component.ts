import { Component, OnInit, ViewChild, ElementRef, AfterViewInit, OnDestroy, QueryList, ViewChildren, Injectable } from '@angular/core';
...
@Component({
  selector: 'app-fju0620,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fju0620.component.html',
  styleUrls: ['./fju0620.component.css']
})


export class Fju0620Component implements AfterViewInit, OnInit, OnDestroy {
  //form: FormGroup;
  title = 'datatables';
  ...
  changeSendTo(event:any){
    // console.log(event);
    if(event == 1){
      this.result.dep_id = this.userData.userCode;
      this.result.dep_name = this.userData.offName;
    }else if(event == 2){
      this.result.dep_id = this.userData.depCode;
      this.result.dep_name = this.userData.depName;
    }else if(event == 3){
  ...
  receiveFuncListData(event:any){
    ...
    if(this.modalType == 1){
      ...
      this.result.sdep_id = event.fieldIdValue;
      this.result.sdep_name1 = event.fieldNameValue;
    }else if(this.modalType == 3){
  ...
  receiveFuncJudgeListData(event:any) {
    ...
    if(this.modalType == 1){
      ...
      this.result.sdep_name1 = event.judge_name;
    }
    this.closebutton.nativeElement.click();
  }
  ...
  getData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.post('/'+this.userData.appName+'ApiJU/API/JUDGEMENT/fju0620/getData', student).subscribe(posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          if(productsJson.result==1){
            this.checklist = this.posts;
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0620 = false);
            this.rerender();
          }else{
  ...
  submitForm() {//save
    ...
    if(!this.result.dep_id){
      ...
      this.http.disableHeader().post('/'+this.userData.appName+'ApiJU/API/JUDGEMENT/fju0620/insertHistoricalData', tmpData ).subscribe(
        (response) =>{
    ...
    if(!chkForm.password){
      ...
      this.http.disableLoading().post('/'+this.userData.appName+'Api/API/checkPassword', student).subscribe(
        posts => {
          ...
          if(productsJson.result==1){
            const confirmBox = new ConfirmBoxInitializer();
            confirmBox.setTitle('ต้องการยกเลิกการรับซองคำพิพากษาที่เลือกใช่หรือไม่?');
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
              if (resp.success==true){
                ...
                if(bar){
                  ...
                  this.http.post('/'+this.userData.appName+'ApiJU/API/JUDGEMENT/fju0620/cancelHistoricalData', data ).subscribe(
                  (response) =>{
                    let alertMessage : any = JSON.parse(JSON.stringify(response));
                    if(alertMessage.result==0){
                      this.SpinnerService.hide();
  ...
  searchCase(){//ลำดับที่ซอง
    ...
    if( this.result.case_running){
      ...
      this.http.disableLoading().post('/' + this.userData.appName + 'ApiJU/API/JUDGEMENT/fju0620/getCaseData', student).subscribe(posts => {
        let productsJson: any = JSON.parse(JSON.stringify(posts));
        if (productsJson.result == 1) {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0620 = this.masterSelected;
      }
  }

  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0620 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0620 = false;
    }
  ...
  hidHoldCase(){
    this.sFind = !this.sFind;
    this.sHold = !this.sHold;

  }

  searchData(type:any){
    
    if(type == 1){//แสดงข้อมูลตามวันที่รับ
    ...
    }else if(type == 2){//แสดงข้อมูลตามหน่วยงานที่ส่ง 
    ...
    this.http.disableLoading().post('/' + this.userData.appName + 'ApiJU/API/JUDGEMENT/fju0620/getData', student).subscribe(posts => {
      let productsJson: any = JSON.parse(JSON.stringify(posts));
      if (productsJson.result == 1) {
        this.posts = productsJson.data;
        this.checklist = this.posts;
