...
@Component({
  selector: 'app-ffn0420,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './ffn0420.component.html',
  styleUrls: ['./ffn0420.component.css']
})

export class Ffn0420Component implements AfterViewInit, OnInit, OnDestroy {
  myExtObject :any;
  sessData:any;
  userData:any;
  programName:any;
 ...
 ngAfterViewInit(): void {
                  ...
                  const subscription2 = confirmBox2.openConfirmBox$().subscribe(resp => {
                    if (resp.success==true){
                      ...
                      var dataDel = [],dataTmp=[];
                          ...
                          console.log(data)
                          this.http.post('/'+this.userData.appName+'ApiFN/API/FINANCE/ffn0420/delData', data ).subscribe(
                            (response) =>{
  ...
  printCheque(event:any){
    ...
    var paramData = JSON.stringify({
      "ppayback_running" : this.queryParams.payback_running,
    ...
    loadData(){
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiFN/API/FINANCE/ffn0420/getData', student ).subscribe(
        (response) =>{
          let alertMessage : any = JSON.parse(JSON.stringify(response));
          console.log(alertMessage)
          if(alertMessage.result==1){
            if(alertMessage.data.length){
              ...
              if(this.queryParams.check_running){
                var filter = this.items.filter((x:any) => x.check_running === parseInt(this.queryParams.check_running));
                if(filter.length)
    ...
    saveData(){
        const confirmBox = new ConfirmBoxInitializer();
        confirmBox.setTitle('ข้อความแจ้งเตือน');
      if(!this.result.check_no){
        confirmBox.setMessage('ป้อนข้อมูลเลขที่เช็ค');
        confirmBox.setButtonLabels('ตกลง');
        confirmBox.setConfig({
            layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
        });
      ...
      }else if(this.result.check_running && this.result.cancel_flag && !this.result.log_remark){
        confirmBox.setMessage('กรุณา login เพื่อยกเลิกเช็ค');
        confirmBox.setButtonLabels('ตกลง');
        confirmBox.setConfig({
            layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
        });
      ...
      }else if(this.queryParams.rcv_amt && parseFloat(this.queryParams.rcv_amt)!=parseFloat(this.result.check_amt)){
        confirmBox.setMessage('ยอดเงินในเช็คไม่เท่ากับยอดเงินที่จ่ายจริง กรุณาตรวจสอบข้อมูลอีกครั้งหลังบันทึก');
        confirmBox.setButtonLabels('ตกลง');
        confirmBox.setConfig({
            layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
        });
    ...
    saveDataCommit(){
      ...
      student['rev_name'] = this.result.pay_name;
      ...
      this.http.post('/'+this.userData.appName+'ApiFN/API/FINANCE/ffn0420/saveData', student ).subscribe(
        (response) =>{
          let getDataOptions : any = JSON.parse(JSON.stringify(response));
          ...
          if(getDataOptions.result==1){
              ...
              const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  this.setDefPage();
                  this.loadData();
                  this.removeParam("check_running", window.location.href);
                }
                subscription.unsubscribe();
              });
              //-----------------------------//
