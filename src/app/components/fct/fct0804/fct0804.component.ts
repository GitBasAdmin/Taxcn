...
@Component({
  selector: 'app-fct0804,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0804.component.html',
  styleUrls: ['./fct0804.component.css']
})


export class Fct0804Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0804?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          ...
          if(productsJson.result==1){
            ...
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0804 = false);
            this.rerender();
          }else{
  ...
  //    )
    ...
    // )
      ...
      //  this.getConcType = [{id:'',text:''},{id: '1',text: 'จังหวัด'},{id: '2',text: 'ผู้ให้คำปรึกษา'}];
        this.getAppealType =  [{id:'',test:''},{id:1,text:'อุทธรณ์คำสั่ง'},{id:2,text:'อุทธรณ์คำพิพากษา'},{id:3,text:'ฏีกาคำพิพากษา'},{id:4,text: 'ฏีกาคำสั่ง'},{id:5,text:'คำร้อง'}];
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0804 = this.masterSelected;
      }
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0804 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0804 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
...
// }
  ...
  submitForm() {
    ...
    if(!this.app_type_id){
      confirmBox.setMessage('กรุณาระบุรหัสประเภทอุทธรณ์คำสั่ง');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
      const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
        if (resp.success==true){
          //this.SpinnerService.hide();
          this.setFocus('app_type_id');
        }
        subscription.unsubscribe();
      });

    }else if(!this.app_type_name){
    ...
    //  }
      ...
      var student = JSON.stringify({
      "edit_app_type_id" : this.hid_app_type_id,
      "edit_appeal_type" : this.hid_appeal_type,
      ...
      this.SpinnerService.show();
      if(this.hid_app_type_id){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0804/updateJson', student , {headers:headers}).subscribe(
          (response) =>{
        ...
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0804/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
...
/*
  editData(val :any) {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    this.SpinnerService.show();
    // alert('/'+this.userData.appName+'ApiCT/API/fct0804/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular')
    this.http.get('/'+this.userData.appName+'ApiCT/API/fct0804/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular').subscribe(edit => {
      //console.log(edit)
      this.SpinnerService.hide();
...
*/
  editData(index:any){
    ...
    // this.renderer.setProperty(this.prov_id.nativeElement['prov_id'],'value', this.posts[index]['prov_id']);
      ...
      this.hid_app_type_id  = this.posts[index]['edit_app_type_id'];
      this.hid_appeal_type  = this.posts[index]['edit_appeal_type'];
      this.app_type_id = this.posts[index]['app_type_id'];
      this.appeal_type = this.posts[index]['appeal_type'];
      this.app_type_name = this.posts[index]['app_type_name'];
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0804/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   console.log(data)
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct0804/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
