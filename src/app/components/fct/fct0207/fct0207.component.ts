...
@Component({
  selector: 'app-fct0207,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0207.component.html',
  styleUrls: ['./fct0207.component.css']
})


export class Fct0207Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if(this.prov_id){
      ...
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0207?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          ...
          if(productsJson.result==1){
            ...
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0207 = false);
            this.rerender();
          }else{
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0207 = this.masterSelected;
      }
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0207 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0207 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
...
// }
  ...
  submitForm() {
    ...
    if(!this.prov_id){
      confirmBox.setMessage('กรุณาระบุรหัสตำบล');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
    ...
    //  }
      ...
      var student = JSON.stringify({
      "edit_tambon_id" : this.hid_tambon_id,
      ...
      "tambon_name" : this.tambon_name,
      "tambon_name_eng" : this.tambon_name_eng,
      "prov_id" : this.prov_id,
      "post_code" : this.post_code,
      "post_remark" : this.post_remark,
      "std_id" : this.std_id,
      "userToken" : this.userData.userToken
      });
      console.log(student)
      ...
      this.SpinnerService.show();
      if(this.hid_prov_id){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0207/updateJson', student , {headers:headers}).subscribe(
          (response) =>{
        ...
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0207/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
...
/*
  editData(val :any) {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    this.SpinnerService.show();
    // alert('/'+this.userData.appName+'ApiCT/API/fct0207/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular')
    this.http.get('/'+this.userData.appName+'ApiCT/API/fct0207/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular').subscribe(edit => {
      //console.log(edit)
      this.SpinnerService.hide();
...
*/
  editData(index:any){
    ...
    // this.renderer.setProperty(this.prov_id.nativeElement['prov_id'],'value', this.posts[index]['prov_id']);
      ...
      this.hid_tambon_id  = this.posts[index]['edit_tambon_id'];
      ...
      this.tambon_name = this.posts[index]['tambon_name'];
      this.tambon_name_eng = this.posts[index]['tambon_name_eng'];
      this.post_code = this.posts[index]['post_code'];
      this.post_remark = this.posts[index]['post_remark'];
      this.std_id = this.posts[index]['std_id'];
      if(this.std_id>0){
        console.log(this.posts[index]['std_id'])
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0207/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if(resp.success==true){
                  ...
                  if(bar){
                   ...
                   //console.log(data)
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct0207/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
