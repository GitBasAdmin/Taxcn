...
@Component({
  selector: 'app-fct0210,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0210.component.html',
  styleUrls: ['./fct0210.component.css']
})


export class Fct0210Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0210?userToken='+this.userData.userToken+':angular').subscribe(posts => {
 ...
 ngAfterViewInit(): void {
  ...
  submitForm() {
    ...
    if(!this.result_id.nativeElement["result_id"].value){
      confirmBox.setMessage('กรุณาระบุรหัสคำสั่งรับฟ้อง');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
      ...
      confirmBox.setMessage('กรุณาระบุคำสั่งรับฟ้อง');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
      ...
      if(this.result_id.nativeElement["hid_result_id"].value){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0210/updateJson', student , {headers:headers}).subscribe(
          (response) =>{
        ...
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0210/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
  ...
  editData(val :any) {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    this.SpinnerService.show();
    //console.log('/'+this.userData.appName+'ApiCT/API/fct0210/edit?edit_result_id='+val+'&userToken='+this.userData.userToken+':angular')
    this.http.get('/'+this.userData.appName+'ApiCT/API/fct0210/edit?edit_result_id='+val+'&userToken='+this.userData.userToken+':angular').subscribe(edit => {
      //console.log(edit)
      this.SpinnerService.hide();
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0210/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
        ...
        posts => {
          ...
          if(productsJson.result==1){
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  this.SpinnerService.show();
                  this.http.get('/'+this.userData.appName+'ApiCT/API/fct0210/delete?delete_result_id='+this.delValue+'&userToken='+this.userData.userToken+':angular').subscribe(del => {
                  console.log(del);
