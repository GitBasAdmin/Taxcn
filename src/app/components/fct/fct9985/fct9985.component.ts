...
@Component({
  selector: 'app-fct9985,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct9985.component.html',
  styleUrls: ['./fct9985.component.css']
})


export class Fct9985Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  loadData(){
    ...
    this.http.post('/'+this.userData.appName+'ApiCT/API/fct9985',student).subscribe(posts => {
      let productsJson : any = JSON.parse(JSON.stringify(posts));
      console.log(productsJson)
      this.posts = productsJson.data;
      this.objLink = this.posts;
      console.log('objLink==>', this.objLink);
      console.log("Iconssssss==>",this.icons.iconName);
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit9985 = this.masterSelected;
      }
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit9985 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit9985 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
...
// }
  ...
  submitForm() {
    // alert(this.system_url);
    const confirmBox = new ConfirmBoxInitializer();
    confirmBox.setTitle('ข้อความแจ้งเตือน');

    if(!this.system_id){
      confirmBox.setMessage('กรุณาระบุรหัสระบบงาน');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
      const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
        if (resp.success==true){
          //this.SpinnerService.hide();
          this.setFocus('system_id');
        }
        subscription.unsubscribe();
      });

    }else if(!this.system_name){
      confirmBox.setMessage('กรุณาระบุชื่อระบบงาน');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
    ...
    //  }
      ...
      var student = JSON.stringify({
      "edit_system_id" : this.hid_system_id,
      "system_id" : this.system_id,
      "system_name" : this.system_name,
      "system_short_name" : this.system_short_name,
      ...
      if(this.hid_system_id){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct9985/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
  ...
  editData(index:any){
    ...
    this.SpinnerService.show();
      ...
      this.hid_system_id  = (this.posts[index]['edit_system_id'] ? this.posts[index]['edit_system_id'] : 0);
      this.system_id = this.posts[index]['system_id'];
      this.system_name = this.posts[index]['system_name'];
      this.system_short_name = this.posts[index]['system_short_name'];
      this.system_icon = this.posts[index]['system_icon'];
      this.system_url = this.posts[index]['system_url'];
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct9985/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
          if(productsJson.result==1){
            ...
            this.checklist2 = this.posts;
              if(productsJson.length)
                this.posts.forEach((x : any ) => x.edit9985 = false);
              this.rerender();
          }else{
            confirmBox.setMessage(productsJson.error);
            confirmBox.setButtonLabels('ตกลง');
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   console.log(data)
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct9985/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
  ...
  getData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.SpinnerService.show();
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct9985?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          console.log(productsJson)
          this.posts = productsJson.data;
