...
import { Component, OnInit, ViewChild, ElementRef, AfterViewInit, OnDestroy, Injectable, Renderer2, Input } from '@angular/core';
...
@Component({
  selector: 'app-fct0406,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0406.component.html',
  styleUrls: ['./fct0406.component.css']
})


export class Fct0406Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  draft_doc_no:any;
  order_no:any;
  print_doc_no:any;getPrintDocNo:any;
  flag_print:any;getFlagPrint:any;
  print_date:any;getPrintDate:any;
  in_out:any;getInOut:any;
  run_group_id:any;
  barcode_id:any;
  dep_use:any;dep_name:any;
  hid_doc_title_id:any;doc_title_id:any;
  doc_title:any;doc_title_desc:any;
  Datalist:any;
  defaultCaseType:any;
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0406?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          ...
          if(productsJson.result==1){
            ...
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0406 = false);
            this.rerender();
          }else{
   ...
   this.http.post('/'+this.userData.appName+'ApiUTIL/API/popup', student , {headers:headers}).subscribe(
    ...
    // )
      ...
      //  this.getConcType = [{id:'',text:''},{id: '1',text: 'จังหวัด'},{id: '2',text: 'ผู้ให้คำปรึกษา'}];
        this.getInOut =  [{id:1,text:'รับหนังสือ'},{id: 2,text: 'ส่งหนังสือ'}];
        this.getPrintDate =  [{id:1,text:'พิมพ์'},{id: 0,text: 'ไม่พิมพ์'}];
        this.getPrintDocNo =  [{id:1,text:'พิมพ์'},{id: 0,text: 'ไม่พิมพ์'}];
        this.getFlagPrint =  [{id:1,text:'พิมพ์'},{id: 0,text: 'ไม่พิมพ์'}];
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0406 = this.masterSelected;
      }
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0406 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0406 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
  ...
  receiveFuncListData(event:any){
    this.dep_use = event.fieldIdValue;
...
// }
  ...
  submitForm() {
    ...
    if(!this.doc_title){
      confirmBox.setMessage('กรุณาระบุรหัสเลขที่ส่งหนังสือ');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
      const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
        if (resp.success==true){
          //this.SpinnerService.hide();
          this.setFocus('doc_title');
        }
        subscription.unsubscribe();
      });

    }else if(!this.doc_title_desc){
     ...
     if(this.draft_doc_no==true){
      ...
      var student = JSON.stringify({
      "edit_doc_title_id" : this.hid_doc_title_id,
      "doc_title_id" : this.doc_title_id,
      "doc_title" : this.doc_title,
      "doc_title_desc" : this.doc_title_desc,
      "draft_doc_no" : inputchk,
      "dep_use" : this.dep_use,
      "in_out" : this.in_out,
      "flag_print" : this.flag_print,
      "print_date" : this.print_date,
      "print_doc_no" : this.print_doc_no,
      "barcode_id" : this.barcode_id,
      "run_group_id" : this.run_group_id,
      "order_no" : this.order_no,
       // "" : this.getTitle.find((x:any) => x.fieldIdValue === this.title).fieldNameValue,
      "userToken" : this.userData.userToken
      });
      console.log(student)

      this.SpinnerService.show();
      if(this.hid_doc_title_id){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0406/updateJson', student , {headers:headers}).subscribe(
          (response) =>{
        ...
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0406/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
...
/*
  editData(val :any) {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    this.SpinnerService.show();
    // alert('/'+this.userData.appName+'ApiCT/API/fct0406/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular')
    this.http.get('/'+this.userData.appName+'ApiCT/API/fct0406/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular').subscribe(edit => {
      //console.log(edit)
      this.SpinnerService.hide();
...
*/
  editData(index:any){
    ...
    // this.renderer.setProperty(this.prov_id.nativeElement['prov_id'],'value', this.posts[index]['prov_id']);
      ...
      this.hid_doc_title_id  = this.posts[index]['edit_doc_title_id'];
      this.doc_title_id = this.posts[index]['doc_title_id'];
      this.doc_title = this.posts[index]['doc_title'];
      this.doc_title_desc = this.posts[index]['doc_title_desc'];
      this.dep_use = this.posts[index]['dep_use'];
      this.dep_name = this.posts[index]['dep_name'];
      this.flag_print = this.posts[index]['flag_print'];
      this.draft_doc_no = this.posts[index]['draft_doc_no'];
      this.in_out = this.posts[index]['in_out'];
      this.run_group_id = this.posts[index]['run_group_id'];
      this.barcode_id = this.posts[index]['barcode_id'];
      this.print_date = this.posts[index]['print_date'];
      this.print_doc_no = this.posts[index]['print_doc_no'];
      if(this.posts[index]['order_no']===0){
         this.order_no = '';
      }else{
        this.order_no = this.posts[index]['order_no'];
      }
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      var student = JSON.stringify({
        ...
        "barcode_id" : this.barcode_id,
        "in_out" : this.in_out,
        "print_date" : this.print_date ? this.print_date : 99,
        "print_doc_no" : this.print_doc_no ? this.print_doc_no : 99,
        "flag_print" : this.flag_print ? this.flag_print : 99,
        "draft_doc_no" : this.draft_doc_no ? 1 : 0,
        "order_no" : this.order_no,
        "userToken" : this.userData.userToken
      });
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0406/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   console.log(data)
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct0406/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
 ...
 tabChange(obj:any){
   if(obj.target.value){
     this.dep_name = this.list.find((x:any) => x.fieldIdValue === parseInt(obj.target.value)).fieldNameValue;
    //  $('[ng-model="doc_title_id"]')
   }else{
     this.dep_use="";
     this.dep_name="";
   }
