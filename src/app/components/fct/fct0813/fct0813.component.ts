...
@Component({
  selector: 'app-fct0813,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0813.component.html',
  styleUrls: ['./fct0813.component.css']
})


export class Fct0813Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0813?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          ...
          if(productsJson.result==1){
            ...
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0813 = false);
            this.rerender();
          }else{
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0813 = this.masterSelected;
      }
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0813 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0813 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
  ...
  receiveFuncListData(event:any){
    this.req_type = event.fieldIdValue;
    this.req_type_name = event.fieldNameValue;
...
// }
  ...
  submitForm() {
    ...
    if(!this.req_doc_title){
      confirmBox.setMessage('กรุณาระบุรหัสคำนำหน้าเอกสารหมาย');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
    ...
    //  }
      ...
      var student = JSON.stringify({
      "edit_req_type_id" : this.hid_req_type_id,
      "req_type_id" : this.req_type_id,
      "req_type" : this.req_type,
      "req_type_name" : this.req_type_name,
      "req_doc_title" : this.req_doc_title,
      ...
      this.SpinnerService.show();
      if(this.hid_req_type_id){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0813/updateJson', student , {headers:headers}).subscribe(
          (response) =>{
        ...
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0813/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
...
/*
  editData(val :any) {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    this.SpinnerService.show();
    // alert('/'+this.userData.appName+'ApiCT/API/fct0813/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular')
    this.http.get('/'+this.userData.appName+'ApiCT/API/fct0813/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular').subscribe(edit => {
      //console.log(edit)
      this.SpinnerService.hide();
...
*/
  editData(index:any){
    ...
    // this.renderer.setProperty(this.prov_id.nativeElement['prov_id'],'value', this.posts[index]['prov_id']);
      ...
      this.hid_req_type_id  = this.posts[index]['edit_req_type_id'];
      this.req_type_id = this.posts[index]['req_type_id'];
      this.req_type = this.posts[index]['req_type'];
      this.req_type_name = this.posts[index]['req_type_name'];
      this.req_doc_title = this.posts[index]['req_doc_title'];
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0813/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   console.log(data)
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct0813/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
 ...
 tabChange(obj:any){
   if(obj.target.value){
     this.req_type_name = this.list.find((x:any) => x.fieldIdValue === parseInt(obj.target.value)).fieldNameValue;
    //  $('[ng-model="req_doc_title"]')
   }else{
     this.req_type="";
     this.req_type_name="";
   }
