...
@Component({
  selector: 'app-fct0443,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0443.component.html',
  styleUrls: ['./fct0443.component.css']
})


export class Fct0443Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0443?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          ...
          if(productsJson.result==1){
            ...
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0443 = false);
            this.rerender();
          }else{
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0443 = this.masterSelected;
      }
  ...
  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0443 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
...
// }
  ...
  submitForm() {
    ...
    if(!this.translator_seq){
      confirmBox.setMessage('กรุณาระบุข้อมูลลำดับ');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
    ...
    //  }
      ...
      var student = JSON.stringify({
      "edit_translator_id" : this.hid_translator_id,
      "translator_id" : this.translator_id,
      "translator_type" : this.translator_type,
      "translator_seq" : this.translator_seq,
      "name" : this.name,
      ...
      this.SpinnerService.show();
      if(this.hid_translator_id){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0443/updateJson', student , {headers:headers}).subscribe(
          (response) =>{
        ...
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0443/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
...
/*
  editData(val :any) {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    this.SpinnerService.show();
    // alert('/'+this.userData.appName+'ApiCT/API/fct0443/edit?edit_conciliate_id='+val+'&userToken='+this.userData.userToken+':angular')
    this.http.get('/'+this.userData.appName+'ApiCT/API/fct0443/edit?edit_conciliate_id='+val+'&userToken='+this.userData.userToken+':angular').subscribe(edit => {
      //console.log(edit)
      this.SpinnerService.hide();
      ...
      if(productsJson.result==0){
        ...
        this.renderer.setProperty(this.conciliate_id.nativeElement['hid_translator_id'],'value', productsJson.data[0]['edit_conciliate_id']);
...
*/
  editData(index:any){
    ...
    this.SpinnerService.show();
    //this.renderer.setProperty(this.conciliate_id.nativeElement['hid_translator_id'],'value', this.posts[index]['edit_conciliate_id']);
    // this.renderer.setProperty(this.conciliate_id.nativeElement['conciliate_id'],'value', this.posts[index]['conciliate_id']);
      //this.translator_type = this.getTranslator.filter((x:any) => x.id === this.posts[index]['translator_type']).text;
      this.hid_translator_id = this.posts[index]['translator_id'];
      this.translator_id = this.posts[index]['translator_id'];
      this.translator_type = this.posts[index]['translator_type'].toString();
      this.translator_seq = this.posts[index]['translator_seq'];
      this.name = this.posts[index]['name'];
      this.id_card = this.posts[index]['id_card'];
      this.addr_no = this.posts[index]['addr_no'];
      this.building = this.posts[index]['building'];
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0443/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct0443/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
