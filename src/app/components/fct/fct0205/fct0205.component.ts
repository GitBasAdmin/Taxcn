...
@Component({
  selector: 'app-fct0205,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0205.component.html',
  styleUrls: ['./fct0205.component.css']
})


export class Fct0205Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  public listFieldName2:any = "std_prov_name";
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0205?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          ...
          if(productsJson.result==1){
            ...
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0205 = false);
            this.rerender();
          }else{
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0205 = this.masterSelected;
      }
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0205 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0205 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
  ...
  receiveFuncListData(event:any){
    ...
    this.std_prov_name = event.fieldNameValue2;
...
// }
  ...
  submitForm() {
    ...
    if(!this.prov_id){
      confirmBox.setMessage('กรุณาระบุรหัสจังหวัด');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
    ...
    //  }
      ...
      "prov_name_eng" : this.prov_name_eng,
      "part" : this.part,
      "std_id" : this.std_id,
      "userToken" : this.userData.userToken
      });
      ...
      this.SpinnerService.show();
      if(this.hid_prov_id){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0205/updateJson', student , {headers:headers}).subscribe(
          (response) =>{
        ...
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0205/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
...
/*
  editData(val :any) {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    this.SpinnerService.show();
    // alert('/'+this.userData.appName+'ApiCT/API/fct0205/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular')
    this.http.get('/'+this.userData.appName+'ApiCT/API/fct0205/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular').subscribe(edit => {
      //console.log(edit)
      this.SpinnerService.hide();
...
*/
  editData(index:any){
    ...
    // this.renderer.setProperty(this.prov_id.nativeElement['prov_id'],'value', this.posts[index]['prov_id']);
      ...
      this.prov_name_eng = this.posts[index]['prov_name_eng'];
      this.part = this.posts[index]['part'];
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0205/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   console.log(data)
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct0205/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
