...
@Component({
  ...
  selector: 'app-fct0461,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0461.component.html',
  styleUrls: ['./fct0461.component.css']
})


export class Fct0461Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    var student = JSON.stringify({
      "table_name" : "pword_parameter_setup",
      "field_id" : "parameter_id",
      "field_name" : "parameter_desc",
      "order_by" : " parameter_id DESC",
  ...
  loadData(){
    ...
    this.http.post('/'+this.userData.appName+'ApiCT/API/fct0461',student).subscribe(posts => {
 ...
 ngAfterViewInit(): void {
  ...
  submitForm() {
    ...
    if(!this.result.parameter_id){
      confirmBox.setMessage('กรุณาระบุใช้กับหน้าจอ');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
    ...
    }else if(!this.result.parameter_variable){
      confirmBox.setMessage('กรุณาระบุตัวแปร');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0461/saveJson', student ).subscribe(
        (response) =>{
          let getDataOptions : any = JSON.parse(JSON.stringify(response));
          console.log(getDataOptions)
          if(getDataOptions.result==1){
              ...
              const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  this.result.edit_parameter_running = getDataOptions.parameter_running;
                  this.loadData();
                }
                subscription.unsubscribe();
              });
  ...
  delData(){
    ...
    if(bar){
      if(del){
        const modalRef = this.ngbModal.open(ModalConfirmComponent,{ windowClass: 'fct0461-conf'})
            ...
            confirmBox.openConfirmBox$().subscribe(resp => {
              if (resp.success==true){
                ...
                if(bar){
                  ...
                  console.log(data)
                  this.http.post('/'+this.userData.appName+'ApiCT/API/fct0461/deleteSelect', data ).subscribe(
                    (response) =>{
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      var student = JSON.stringify({
        "parameter_id" : this.result.parameter_id,
        "parameter_desc" : this.result.parameter_desc,
        "parameter_variable" : this.result.parameter_variable,
        "userToken" : this.userData.userToken
      });
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0461/search',student).subscribe(posts => {
        let productsJson : any = JSON.parse(JSON.stringify(posts));
        console.log(productsJson)
        this.posts = productsJson.data;
        if(productsJson.result==1){
