...
@Component({
  selector: 'app-fct0509,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0509.component.html',
  styleUrls: ['./fct0509.component.css']
})


export class Fct0509Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  @ViewChild( DatalistReturnMultipleDepComponent ) child2: DatalistReturnMultipleDepComponent ;
  ...
  ngOnInit(): void {
    ...
    var student = JSON.stringify({
      ...
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0509',student).subscribe(response => {
          ...
          if(productsJson.result==1){
            ...
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0509 = false);
            this.rerender();
          }else{
    ...
    this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student ).subscribe(
      ...
      // //======================== pDep ======================================
      // var student = JSON.stringify({
      //   "table_id" :  this.hid_table_id,
      //   "userToken" : this.userData.userToken
      // });
      // console.log(student)
      // this.http.post('/'+this.userData.appName+'ApiCT/API/fct0509/getCaseCateMapTable', student ).subscribe(
      //   (response) =>{
      //     let getDataOptions : any = JSON.parse(JSON.stringify(response));
      //     // getDataOptions.unshift({fieldIdValue:0,fieldNameValue: 'ทั้งหมด'});
      //     this.getCaseCate = getDataOptions;
  ...
  clickOpenMyModalComponent(type:any,depid:any,tableid:any){
    console.log(depid);
    this.modalType = type;
    this.listDepId = depid;
    this.hid_table_id = tableid;
  ...
  //   }
    ...
    loadMyModalComponent(){
      if(this.modalType==1){
        ...
        console.log(student)
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0509/getDepMapTable', student ).subscribe(
          (response) =>{
            console.log(response)
            let productsJson : any = JSON.parse(JSON.stringify(response));
           // this.list = response;
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0509 = this.masterSelected;
      }
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0509 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0509 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
  ...
  */
  ...
  submitForm() {
    ...
    if(!this.result.table_id){
      ...
      const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
        if (resp.success==true){
          //this.SpinnerService.hide();
          $("input[name='table_id']")[0].focus();
        }
        subscription.unsubscribe();
      });

    }else if(!this.result.table_name){
    ...
    //  }
      ...
      if(this.hid_table_id){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0509/saveData', student ).subscribe(
          (response) =>{
  ...
  editData(index:any){
    ...
    //this.judge_name = this.getJudge.filter((x:any) => x.fieldIdValue === this.posts[index]['prov_id']).fieldNameValue;
      ...
      this.hid_table_id  = this.posts[index]['edit_table_id'];
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0509', student , {headers:headers}).subscribe(
        posts => {
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct0509/deleteDataSelect', data ).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
  ...
  depData(){
    ...
    // this.tmpResult.forEach((element,index,Array) =>
    //     // console.log(element,index)
    //     this.dataDep.push({'dep_id':element})
    // );
    // console.log(this.dataDep);
    var jsonTmp = [],depTmp = [];
    console.log(this.dataDep);
    depTmp = this.dataDep;
    console.log(depTmp);
    jsonTmp['table_id'] = this.hid_table_id;
    jsonTmp['dep_data'] = $.extend([],depTmp);
    jsonTmp['userToken'] = this.userData.userToken;

    var student = $.extend({},jsonTmp);

    console.log(JSON.stringify(student))
    this.http.post('/'+this.userData.appName+'ApiCT/API/fct0509/saveDepData', student ).subscribe(
      (response) =>{
          let alertMessage : any = JSON.parse(JSON.stringify(response));
          console.log(alertMessage)
          if(alertMessage.result==0){
 ...
 tabChange(obj:any){
  ...
  getData(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.SpinnerService.show();
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0509?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          console.log(productsJson)
          this.posts = productsJson.data;
          console.log(this.posts);
