...
@Component({
  selector: 'app-fct0235,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0235.component.html',
  styleUrls: ['./fct0235.component.css']
})


export class Fct0235Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0235?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          ...
          if(productsJson.result==1){
            ...
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0235 = false);
            this.rerender();
          }else{
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0235 = this.masterSelected;
      }
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0235 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0235 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
...
// }
  ...
  submitForm() {
    ...
    //  }
      ...
      "edit_title_flag" : this.hid_title_flag,
      "edit_title" : this.hid_title,
      "dep_code" : this.dep_code,
      "case_type" : this.case_type,
      "title_flag" : this.title_flag,
      ...
      if(this.hid_dep_code){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0235/updateJson', student , {headers:headers}).subscribe(
          (response) =>{
        ...
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0235/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
            console.log(alertMessage)
            if(alertMessage.result==0){
...
/*
  editData(val :any) {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    this.SpinnerService.show();
    // alert('/'+this.userData.appName+'ApiCT/API/fct0235/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular')
    this.http.get('/'+this.userData.appName+'ApiCT/API/fct0235/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular').subscribe(edit => {
      //console.log(edit)
      this.SpinnerService.hide();
...
*/
  editData(index:any){
    ...
    // this.renderer.setProperty(this.prov_id.nativeElement['prov_id'],'value', this.posts[index]['prov_id']);
      ...
      this.hid_case_type  = this.posts[index]['edit_case_type'];
      this.hid_title_flag  = this.posts[index]['edit_title_flag'];
      this.hid_title  = this.posts[index]['edit_title'];
      this.dep_code = this.posts[index]['dep_code'];
      this.case_type = this.posts[index]['case_type'];
      this.title_flag = this.posts[index]['title_flag'];
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0235/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   console.log(data)
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct0235/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
