...
@Component({
  selector: 'app-fct0320,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fct0320.component.html',
  styleUrls: ['./fct0320.component.css']
})


export class Fct0320Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      this.http.get('/'+this.userData.appName+'ApiCT/API/fct0320?userToken='+this.userData.userToken+':angular').subscribe(posts => {
          ...
          if(productsJson.result==1){
            ...
            if(productsJson.data.length)
              this.posts.forEach((x : any ) => x.edit0320 = false);
            this.rerender();
          }else{
  ...
  //    )
    ...
    // )
      ...
      //  this.getConcType = [{id:'',text:''},{id: '1',text: 'จังหวัด'},{id: '2',text: 'ผู้ให้คำปรึกษา'}];
        this.getNoticeResult =  [{id:'',test:''},{id:1,text:'1.ส่งได้'},{id: 2,text: '2.ส่งไม่ได้'}];
 ...
 ngAfterViewInit(): void {
  ...
  checkUncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0320 = this.masterSelected;
      }
  ...
  isAllSelected() {
    this.masterSelected = this.checklist.every(function(item:any) {
        return item.edit0320 == true;
    })
  }

  uncheckAll() {
    for (var i = 0; i < this.checklist.length; i++) {
      this.checklist[i].edit0320 = false;
    }
    // for (var i = 0; i < this.checklist2.length; i++) {
...
// }
  ...
  submitForm() {
    ...
    if(!this.send_order_id){
      confirmBox.setMessage('กรุณาระบุรหัสคำสั่งศาลสำหรับผลหมาย');
      confirmBox.setButtonLabels('ตกลง');
      confirmBox.setConfig({
          layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
      });
      const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
        if (resp.success==true){
          //this.SpinnerService.hide();
          this.setFocus('send_order_id');
        }
        subscription.unsubscribe();
      });

    }else if(!this.send_order_desc){
    ...
    //  }
      ...
      var student = JSON.stringify({
      "edit_send_order_id" : this.hid_send_order_id,
      "send_order_id" : this.send_order_id,
      "notice_result" : this.notice_result,
      "inout_flag" : this.inout_flag,
      "send_order_desc" : this.send_order_desc,
      ...
      this.SpinnerService.show();
      if(this.hid_send_order_id){
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0320/updateJson', student , {headers:headers}).subscribe(
          (response) =>{
        ...
        this.http.post('/'+this.userData.appName+'ApiCT/API/fct0320/saveJson', student , {headers:headers}).subscribe(
          (response) =>{
            let alertMessage : any = JSON.parse(JSON.stringify(response));
...
/*
  editData(val :any) {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
    this.SpinnerService.show();
    // alert('/'+this.userData.appName+'ApiCT/API/fct0320/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular')
    this.http.get('/'+this.userData.appName+'ApiCT/API/fct0320/edit?edit_prov_id='+val+'&userToken='+this.userData.userToken+':angular').subscribe(edit => {
      //console.log(edit)
      this.SpinnerService.hide();
...
*/
  editData(index:any){
    ...
    // this.renderer.setProperty(this.prov_id.nativeElement['prov_id'],'value', this.posts[index]['prov_id']);
      ...
      this.hid_send_order_id  = this.posts[index]['edit_send_order_id'];
      this.send_order_id = this.posts[index]['send_order_id'];
      this.notice_result = this.posts[index]['notice_result'];
      this.inout_flag = this.posts[index]['inout_flag'];
      this.send_order_desc = this.posts[index]['send_order_desc'];
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiCT/API/fct0320/search', student , {headers:headers}).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   console.log(data)
                   this.http.post('/'+this.userData.appName+'ApiCT/API/fct0320/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
