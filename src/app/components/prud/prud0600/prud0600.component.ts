...
@Component({
  selector: 'app-prud0600,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './prud0600.component.html',
  styleUrls: ['./prud0600.component.css']
})


export class Prud0600Component implements AfterViewInit, OnInit, OnDestroy {
  myExtObject :any;
  //form: FormGroup;
  title = 'datatables';
  ...
  @ViewChild('prud0600',{static: true}) prud0600 : ElementRef;
  ...
  ngOnInit(): void {
    ...
    //----------------------end pcase_type -------------------
    this.getSel1 = [{id:'1',text:'วันที่รับซอง'},{id:'0',text:'ทั้งหมด'}];
    this.getSel2 = [{id:'0',text:'ทั้งหมด'},{id:'1',text:'ซองคำพิพากษาค้างอ่าน'},{id:'2',text:'ซองคำพิพากษาคงค้าง'}];
    this.setDefPage();
  }
  ...
  loadMyModalComponent(){
    ...
    //this.loadModalListComponent = true;
      ...
      modalRef.componentInstance.value1 = "pjudge_case_type"
      modalRef.componentInstance.value2 = "judge_case_type"
      modalRef.componentInstance.value3 = "judge_case_name"
      modalRef.componentInstance.types = "1"

      modalRef.result.then((item: any) => {
        if(item){
          this.result.pjudge_type=item.fieldIdValue;
          this.result.judge_case_name=item.fieldNameValue;
        }
      })
    },
    (error) => {}
    ...
    printReport(){
      ...
      // For set parameter to report
       var paramData = JSON.stringify({
        "pjudge_type" : this.result.pjudge_type ? this.result.pjudge_type.toString() : '0',
        ...
        "psel1" : this.result.psel1 ? this.result.psel1.toString() : '',
        "psel2" : this.result.psel2 ? this.result.psel2.toString() : '',
       });
