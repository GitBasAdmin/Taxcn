...
@Component({
  selector: 'app-fmg2100,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fmg2100.component.html',
  styleUrls: ['./fmg2100.component.css']
})


export class Fmg2100Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student);
      // this.http.get('/'+this.userData.appName+'ApiMG/API/MANAGEMENT/fmg2100/getData?userToken='+this.userData.userToken+':angular').subscribe(posts => {
      this.http.post('/'+this.userData.appName+'ApiMG/API/MANAGEMENT/fmg2100/getData', student ).subscribe(posts => {
 ...
 ngAfterViewInit(): void {
  ...
  submitForm() {
    ...
    //  }
      ...
      if(this.hid_judge_id){
        this.http.post('/'+this.userData.appName+'ApiMG/API/MANAGEMENT/fmg2100/saveData', student ).subscribe(
          (response) =>{
  ...
  searchData() {
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiMG/API/MANAGEMENT/fmg2100/getData', student).subscribe(
        posts => {
          let productsJson : any = JSON.parse(JSON.stringify(posts));
          this.posts = productsJson.data;
          console.log(productsJson)
            ...
            const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
                if (resp.success==true){
                  ...
                  if(bar){
                   ...
                   this.http.post('/'+this.userData.appName+'ApiMG/API/MANAGEMENT/fmg2100/deleteSelect', data , {headers:headers}).subscribe(
                    (response) =>{
                      let alertMessage : any = JSON.parse(JSON.stringify(response));
                      if(alertMessage.result==0){
                        this.SpinnerService.hide();
