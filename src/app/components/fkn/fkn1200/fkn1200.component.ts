...
@Component({
  selector: 'app-fkn1200,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fkn1200.component.html',
  styleUrls: ['./fkn1200.component.css']
})


export class Fkn1200Component implements AfterViewInit, OnInit, OnDestroy {
  //form: FormGroup;
  myExtObject:any;
  title = 'datatables';
  result:any = [];
   ...
   this.http.post('/'+this.userData.appName+'ApiUTIL/API/popupJudge', student).subscribe(
    ...
    this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student).subscribe(
      (response) =>{
        ...
        if(Dep.length!=0){
          this.result.pcreate_dep_code = this.pcreate_dep_code = Dep[0].fieldIdValue;
        }
      },
      (error) => {}
    )
    ...
    this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student).subscribe(
      (response) =>{
        ...
        if(Off.length!=0){
          this.result.puser_id = this.puser_id = Off[0].fieldIdValue;
        }
      },
      (error) => {}
    )

     //======================== pperformance_manual_setup ======================================
     var student = JSON.stringify({
      "table_name" : "pperformance_manual_setup",
      "field_id" : "performance_id",
      "field_name" : "performance_name",
      "order_by": " performance_id ASC",
      "userToken" : this.userData.userToken
    });
    //console.log(student)
    this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', student ).subscribe(
 ...
 loadMyModalComponent(){
  ...
  ngOnDestroy(): void {
    ...
    var bar = new Promise((resolve, reject) => {
      ...
      if (!del) {
        ...
        this.SpinnerService.show();
          ...
          this.http
            .post('/'+this.userData.appName+'ApiST/API/STAT/fkn1200/saveData', data, {
    ...
    searchData(){
      ...
      if(!this.result.performance_id &&
        !this.result.case_type_stat &&
        !this.result.puser_id &&
        !this.result.pcreate_dep_code &&
        !this.result.sdate &&
        !this.result.edate
        ){
          const confirmBox = new ConfirmBoxInitializer();
        ...
        const confirmBox = new ConfirmBoxInitializer();
        confirmBox.setTitle('ข้อความแจ้งเตือน');
        confirmBox.setMessage('ป้อนวันที่ปฏิบัติงานให้ครบ');
        confirmBox.setButtonLabels('ตกลง');
        confirmBox.setConfig({
            layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
        });
        ...
        this.http.post('/'+this.userData.appName+'ApiKN/API/KEEPN/fkn1200', student ).subscribe(
          (response) =>{
              let productsJson : any = JSON.parse(JSON.stringify(response));
    ...
    submitCheck(){
      ...
      var student = jsonTmp;
      console.log(JSON.stringify(student))
      this.http.post('/'+this.userData.appName+'ApiST/API/STAT/fkn1200', student ).subscribe(
        (response) =>{
            let productsJson : any = JSON.parse(JSON.stringify(response));
            if(productsJson.result==1){
              this.loadResult();
  ...
  goToPage(Page:any){
    ...
    winURL = winURL.substring(0,winURL.lastIndexOf("/")+1)+Page;
    //alert(winURL);
  ...
  tabChangeSelectArry(index:any,objName:any,objData:any,event:any,type:any){
    ...
    if(typeof objData!='undefined'){
      ...
      if(objName=='result_id'){
            if(val.length!=0){
              this.dataSearch[index]['edit0210'] = true;
            if(type==1)
