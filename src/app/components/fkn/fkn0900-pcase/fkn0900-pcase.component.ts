...
@Component({
  selector: 'app-fkn0900-pcase,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fkn0900-pcase.component.html',
  styleUrls: ['./fkn0900-pcase.component.css']
})


export class Fkn0900PcaseComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  searchData(student:any){
    ...
    this.http.post('/' + this.userData.appName + 'ApiKN/API/KEEPN/fkn0900/getCaseData', student.sendData,).subscribe(posts => {
        ...
        if (productsJson.result == 1) {
          ...
          this.pData.forEach((ele, index, array) => {
            ...
            ele.fee_desc = ele.fee_desc ? this.curencyFormat(ele.fee_desc, 2) : '';
            ele.court_fee = ele.court_fee ? this.curencyFormat(ele.court_fee, 2) : '';
            ele.short_indict_desc = ele.indict_desc ? ele.indict_desc.substr(1,25) : '';
            ele.tmp_create_date = ele.log_create_date ? myExtObject.conDate(ele.log_create_date) : '';
          });
          console.log(this.pData,this.pData.length)
        } else {
          this.pData=[];
        }
        this.rerender();
