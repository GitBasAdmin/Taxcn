...
@Component({
  ...
  selector: 'app-fap0110-color,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fap0110-color.component.html',
  styleUrls: ['./fap0110-color.component.css']
})

export class Fap0110ColorComponent implements AfterViewInit, OnInit, OnDestroy {
  //form: FormGroup;
  title = 'datatables';
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      this.result.table_id = params['table_id']?params['table_id']:'';
      this.result.month = params['month']?parseInt(params['month']):parseInt(myExtObject.curMonth());
      this.result.year = params['year']?params['year']:myExtObject.curYear();
      if(this.result.table_id && this.result.month && this.result.year){
        this.printColor(params['type']);
      }
    });
...
printColor(val:any){
  ...
  console.log(student)
  this.http.post('/'+this.userData.appName+'ApiAP/API/APPOINT/fap0110/printAppointDisplayColorTable', student ).subscribe(
    (response) =>{
      let productsJson : any = JSON.parse(JSON.stringify(response));
      console.log(productsJson)
      if(productsJson.result==1){
        var bar = new Promise((resolve, reject) => {
          this.displayTable = productsJson.data;
        });
        if(bar){
          this.SpinnerService.hide();
        }
...
public downloadAsPDF() {
  ...
  const documentDefinition = { 
    ...
    styles: {
      ...
      rowOdd: { fillColor:'#FFFFEE'},
      rowEven: { fillColor:'#D4F1D4'},
      disbDay: { fillColor:'#FF6600'},
      sunDay: { fillColor:'#FF0000'},
      backgroundGreen: { fillColor:'#1b7814'},
      backgroundWhite: { fillColor:'#FFFFFF'},

      background_1: { backgroundColor:'#00FFFF',alignment: 'center'},
      background_2: { fillColor:'#33CCFF'},
      background_3: { fillColor:'#00CC33'},
      background_4: { fillColor:'#FF99FF'},
      background_5: { fillColor:'#FFCC33'},
      background_6: { fillColor:'#9933FF'},
      background_7: { fillColor:'#9999CC'},
      background_8: { fillColor:'#FFFF66'},
      background_9: { fillColor:'#CC9966'},
      background_10: { fillColor:'#FF6600'},
      background_11: { fillColor:'#0099FF'},
      background_12: { fillColor:'#33FF66'},
      background_13: { fillColor:'#CCCCFF'},
      background_14: { fillColor:'#CC00CC'},
      background_15: { fillColor:'#6666CC'},
      background_16: { fillColor:'#999999'},
      background_17: { fillColor:'#489100'},
      background_18: { fillColor:'#FF3399'},
      background_19: { fillColor:'#00CECE'},
      background_120: { fillColor:'#3399CC'},
      background_21: { fillColor:'#EA004D'},
      background_22: { fillColor:'#93DCFF'},
      background_23: { fillColor:'#FFCC99'},
      background_24: { fillColor:'#FF51FF'},

      appAlert0: { bold: true,color:'#009900',alignment: 'center'},
      appAlert1: { bold: true,color: '#FF9900',alignment: 'center'},
      appAlert2: { bold: true,color: '#FF0000',alignment: 'center'},
      appAlert3: { bold: true,alignment: 'center'}
    },
    content: html,
    pageSize: 'A1',
