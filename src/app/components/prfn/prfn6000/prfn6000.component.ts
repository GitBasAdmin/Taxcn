...
@Component({
  selector: 'app-prfn6000,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './prfn6000.component.html',
  styleUrls: ['./prfn6000.component.css']
})


export class Prfn6000Component implements AfterViewInit, OnInit, OnDestroy {
  myExtObject :any;
  //form: FormGroup;
  title = 'datatables';
  ...
  @ViewChild('prfn6000',{static: true}) prfn6000 : ElementRef;
  ...
  ngOnInit(): void {
    ...
    this.getType = [{id:'1',text:'พิมพ์เช็ค'},{id:'2',text:'จ่ายเช็ค'}];
    ...
    this.result.preceipt_type_id = 1;
...
this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', Json , {headers:headers}).subscribe(
  (response) =>{
    ...
    this.getRefTitle = getDataOptions;
  },
  (error) => {}
)
...
this.http.post('/'+this.userData.appName+'ApiUTIL/API/getData', Json , {headers:headers}).subscribe(
  (response) =>{
    ...
    let getDataOptions : any = JSON.parse(JSON.stringify(response));
    getDataOptions.unshift({fieldIdValue:0,fieldNameValue: 'ทั้งหมด'});
    this.getCtfinTitle = getDataOptions;
  },
  (error) => {}
)
    this.setDefPage();
...
changeRcvFlag(val:any){
  ...
  printReport(){
    ...
    var paramData = JSON.stringify({
      ...
      "pbook_no" : this.result.pbook_no ? this.result.pbook_no.toString() : '',
      "pcheq_no1" : this.result.pcheq_no1 ? this.result.pcheq_no1.toString() : '',
      "pcheq_no2" : this.result.pcheq_no2 ? this.result.pcheq_no2.toString() : '',
      "pref_title" : this.result.pref_title ? this.result.pref_title.toString() : '',
      "pctfin_title" : this.result.pctfin_title ? this.result.pctfin_title.toString() : '',
    });
