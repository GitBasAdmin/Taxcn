...
@Component({
  selector: 'app-fkb0500,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fkb0500.component.html',
  styleUrls: ['./fkb0500.component.css']
})


export class Fkb0500Component implements AfterViewInit, OnInit, OnDestroy {
  //form: FormGroup;
  title = 'datatables';
  ...
  setDefaultPage(type:any){
    ...
    var dep_name = this.result.dep_name;
    this.result = [];
    ...
    this.result.dep_id = dep_id;
    this.result.dep_name = dep_name;
    if(type==1)
      this.changeCaseType(this.userData.defaultCaseType);
  }
 ...
 ngAfterViewInit(): void {
  ...
  ngOnDestroy(): void {
    ...
    searchCaseNo(type:any){
      ...
      if(type==1){//เลขคดีดำ
        if(!this.result.title){
          ...
          var apiUrl = '/'+this.userData.appName+'ApiKB/API/KEEPB/fkb0500/searchCase';
          //console.log(student)
        }
          ...
          this.http.post(apiUrl, student , {headers:headers}).subscribe(
            (response) =>{
              ...
              if(getDataOptions.result==1){
                ...
                if(this.dataSearch[0].dep_id)
                  this.result.dep_id = this.dataSearch[0].dep_id
                if(this.dataSearch[0].dep_name)
                  this.result.dep_name = this.dataSearch[0].dep_name
                this.rerender();
                if(type==1 || type==2)
                  this.buttonSearch = 1;
                else
    ...
    saveData(){
      ...
      if(!this.result.dep_id){
        ...
        const subscription = confirmBox.openConfirmBox$().subscribe(resp => {
          ...
          this.http.post('/'+this.userData.appName+'ApiKB/API/KEEPB/fkb0500/saveData', student , {headers:headers}).subscribe(
            (response) =>{
              let getDataOptions : any = JSON.parse(JSON.stringify(response));
              console.log(getDataOptions)
              if(getDataOptions.result==1){
    ...
    loadDataCurrent(){
      ...
      console.log(student)
      this.http.post('/'+this.userData.appName+'ApiKB/API/KEEPB/fkb0500', student , {headers:headers}).subscribe(
        datalist => {
    ...
    searchReq(){
      ...
      if(!this.result.req_no){
        ...
        console.log(student)
        this.http.post('/'+this.userData.appName+'ApiKB/API/KEEPB/fkb0500/searchRequest', student , {headers:headers}).subscribe(
          datalist => {
            let getDataOptions : any = JSON.parse(JSON.stringify(datalist));
            console.log(getDataOptions)
            if(getDataOptions.result==1){
                  ...
                  if (resp.success==true){
                      ...
                      if(this.delTypeApp=='cancelData'){
                          ...
                          if(bar){
                            ...
                            console.log(data)
                            this.http.post('/'+this.userData.appName+'ApiKB/API/KEEPB/fkb0500/cancelSelect', data , {headers:headers}).subscribe(
                              (response) =>{
