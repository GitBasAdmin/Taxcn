...
@Component({
  selector: 'app-fsc0600,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fsc0600.component.html',
  styleUrls: ['./fsc0600.component.css']
})


export class Fsc0600Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    searchData(){
      ...
      if(!this.result.send_sdate || !this.result.send_edate){
        ...
        this.http.post('/'+this.userData.appName+'ApiSC/API/NOTICESC/fsc0600', student ).subscribe(
          (response) =>{
              let productsJson : any = JSON.parse(JSON.stringify(response));
    ...
    exportAsXLSX(): void {
      if(this.dataSearch){
        ...
        var bar = new Promise((resolve, reject) => {
          ...
          for(var i = 0; i < excel.length; i++){
            ...
            for(var x=0;x<17;x++){
              ...
              if(x==1)
                data.push(excel[i]['c_no']?excel[i]['c_no']+'/'+excel[i]['yy']:'');
              if(x==2)
                data.push(excel[i]['in_out']);
              if(x==3)
                data.push(excel[i]['police_name']);
              if(x==4)
                data.push(excel[i]['r_name']);
              if(x==5)
                data.push(excel[i]['judge_name']);
              if(x==6)
                data.push(excel[i]['send_date']);
              if(x==7)
                data.push(excel[i]['casedate_to']);
              if(x==8)
                data.push(excel[i]['catchover_date']);
              if(x==9)
                data.push(excel[i]['result_flag']);
              if(x==10)
                data.push(excel[i]['c_no_cancel']);
              if(x==11)
        ...
        if(bar){
          ...
          this.excelService.exportAsExcelFile(objExcel,'fsc0600' ,this.userData.programName);
        }
