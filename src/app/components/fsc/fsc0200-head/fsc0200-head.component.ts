...
@Component({
  selector: 'app-fsc0200-head',
  templateUrl: './fsc0200-head.component.html',
  styleUrls: ['./fsc0200-head.component.scss']
})

export class Fsc0200HeadComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  public loadModalListCatchComponent: boolean = false;
  ...
  clickOpenMyModalComponent(type:any){
    if(this.getCatch){
      this.run_id = this.getCatch.catch_data[0].run_id;
  ...
  loadMyModalComponent(){
    $(".modal-backdrop").remove();
      ...
      this.listFieldName='';
      this.listFieldNull='';
      this.loadModalListCatchComponent = true;
  ...
  closeModal(){
    this.loadModalListCatchComponent = false;
  }
  ...
  loadHead(){
      this.hResult.run_id = this.getCatch.catch_data[0].run_id;
      this.hResult.case_cate_id = this.getCatch.data[0].case_cate_id;
      this.hResult.case_cate_name = this.getCatch.data[0].case_cate_name;
      if(this.getCatch.catch_data[0].sno_from){
        this.hResult.nofrom = this.getCatch.catch_data[0].sno_from ? this.getCatch.catch_data[0].sno_from : '';
        this.hResult.num_nofrom = this.getCatch.catch_data[0].total ? this.getCatch.catch_data[0].total : '';
      }
  }

  // async searchCatchData(): Promise <void>{
    searchCatchData(){
        ...
        //     });
            ...
            if(bar){
              ...
              if(this.hResult.run_id){
                ...
                var num=parseInt(this.hResult.num_nofrom)-parseInt(this.getCatch.catch_data[0].total);
                if(num > 0){
                    dataDel['catch_no_data'] = this.getCatch.catch_data[0].catch_no_data;
                  for (let index = 0; index < num; index++) {
                    dataDel['catch_no_data'].push({"c_seq":0});
                  }
                }else{
            ...
            this.getMessage('ระบุจำนวนหมายจับที่ต้องการ');
           }
      }
    }
  ...
  getHeadNew(){
    ...
    DataHeadNew["catch_data"] = dataTmp;
