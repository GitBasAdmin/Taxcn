...
@Component({
  selector: 'app-fsc0300,ngbd-popover-basic,ngbd-popover-tplcontent',
  templateUrl: './fsc0300.component.html',
  styleUrls: ['./fsc0300.component.css']
})


export class Fsc0300Component implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    searchData(){
      ...
      if(!this.result.send_sdate || !this.result.send_edate){
        ...
        this.http.post('/'+this.userData.appName+'ApiSC/API/NOTICESC/fsc0300', student ).subscribe(
          (response) =>{
              let productsJson : any = JSON.parse(JSON.stringify(response));
    ...
    exportAsXLSX(): void {
      if(this.dataSearch){
        ...
        if(bar){
          ...
          this.excelService.exportAsExcelFile(objExcel,'fsc0300' ,this.userData.programName);
        }
