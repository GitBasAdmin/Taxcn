...
@Component({
  selector: 'app-datalist-return-catch',
  templateUrl: './datalist-return-catch.component.html',
  styleUrls: ['./datalist-return-catch.component.css']
})
export class DatalistReturnCatchComponent implements AfterViewInit,OnInit {
  @Input() items : any = [];
  ...
  getCatchNo(){
    ...
    if(this.formList.get("word_id")?.value){
      var student = JSON.stringify({
        "c_no": parseInt(this.formList.get("word_id")?.value),
        "c_no_yy": parseInt(this.formList.get("word_desc")?.value),
        "userToken" : this.userData.userToken
      });
      console.log(student)
      let promise = new Promise((resolve, reject) => {
        this.http.post('/'+this.userData.appName+'ApiSC/API/NOTICESC/fsc0200/getNoticeCatchData', student).subscribe(
        (response) =>{
