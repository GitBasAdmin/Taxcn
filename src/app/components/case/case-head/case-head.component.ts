import { Component, OnInit , ViewChild , ElementRef,AfterViewInit,OnDestroy,Injectable,Input,Output,EventEmitter,AfterContentInit,OnChanges,SimpleChanges,ViewEncapsulation   } from '@angular/core';
...
import { ModalWebImportComponent } from '@app/components/modal/modal-web-import/modal-web-import.component';
...
@Component({
  ...
  styleUrls: ['./case-head.component.css']
})
...
export class CaseHeadComponent implements AfterViewInit, OnInit, OnDestroy,AfterContentInit,OnChanges {
        ...
        this.http.post(apiUrl, student , {headers:headers}).subscribe(
          (response) =>{
            ...
            if(getDataOptions.data[0]){
              ...
              if(type==1 || type==2){//ถ้าคลิกค้นจากหน้าจอ 1เลขดำ 2เลขแดง
  ...
  webImport(){
    ...
    confirmBox.setTitle('ข้อความแจ้งเตือน');
      ...
      if(!this.dataHead.run_id){
        confirmBox.setMessage('กรุณาค้นหาข้อมูลคดีก่อนการนำเข้าข้อมูลจาก Web');
        confirmBox.setButtonLabels('ตกลง');
        confirmBox.setConfig({
            layoutType: DialogLayoutDisplay.WARNING // SUCCESS | INFO | NONE | DANGER | WARNING
        });
        ...
        const modalRef: NgbModalRef = this.ngbModal.open(ModalWebImportComponent, { windowClass: 'my-class' })
        var student = JSON.stringify({
          "case_type" : this.dataHead.case_type?this.dataHead.case_type.toString():'0',
          "title" : this.dataHead.title,
          "id" : this.dataHead.id?this.dataHead.id.toString():'0',
          "yy" : this.dataHead.yy?this.dataHead.yy.toString():'0',
          "userToken" : this.userData.userToken
        });
        modalRef.componentInstance.obj = student;
        modalRef.componentInstance.type = 1;
        modalRef.componentInstance.run_id = this.dataHead.run_id;
        modalRef.shown.subscribe((data) => {
        });
