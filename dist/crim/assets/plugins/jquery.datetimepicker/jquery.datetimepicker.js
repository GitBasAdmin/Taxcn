...
//						console.log("onShow");
			...
			function getCurrentValue() {
				var ct = false, time,set_Date,new_Date;
				if (options.startDate) {
					ct = _xdsoft_datetime.strToDate(options.startDate);
				} else {
					if(input.val()){
					set_Date = $.trim(input.val()).split("/");
					new_Date = set_Date[0]+"/"+set_Date[1]+"/"+parseInt(set_Date[2]-543);
					//console.log(new_Date)
					}
					//ct = options.value || ((input && input.val && input.val()) ? input.val() : '');
					ct = options.value || new_Date;
...
/*
	var thaiYear = function (ct) {
        ...
        this.setOptions({  
            i18n:{ th:{dayOfWeek:dayWeek}},dayOfWeekStart:leap,setLocale:thai,
        })                
    };  
	var getThaiYear = function (ct) {
        var yearT=new Date(ct).getFullYear()+543;
		console.log()
    };  
	*/